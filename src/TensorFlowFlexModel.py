...
class TensorFlowFlexModel:
  ...
  def train(self):
    ...
    #2025/09/15 Remove model_dir if it exists, and recreate the directory
    model_dir    = self.config.get(ConfigParser.TRAIN, "model_dir", dvalue="./models")
    if os.path.exists(model_dir):
      shutil.rmtree(model_dir)
    os.makedirs(model_dir)
    # 2025/09/15 Added the following line.
  ...
  def predict(self, img):
      ...
      # 2025/09/08 
      #Resize the PIL predicted_rgb_mask to the original image size
      predicted_rgb_mask = predicted_rgb_mask.resize((w, h))
      #print("Resized predicted_rgb_mask w:{}, h:{}".format(w, h))
